...
error_t write_matrix(FILE *fp, const matrix_t *m)
{
  ...
  if (m->fmt != 0) return E_FORMAT_ERROR;
...
error_t dup_matrix(const matrix_t *m_src, matrix_t **m_dst)
{
  ...
  if ((e = create_and_fill_matrix(get_rows(m_src), get_cols(m_src), 0.0, m_dst)) == E_OK){
...
error_t sum_inplace(const matrix_t *m_src, matrix_t *m_dst) // idem sum con la diferencia de que no existe una matriz resultado y lo que se hace es pisar los valores en m_dst
{
  ...
  if (check_dimetions(m_src, get_rows(m_dst), get_cols(m_dst))){
...
int cmp_matrix(const matrix_t *ma, const matrix_t *mb)
{
  if ( check_dimetions(ma, get_rows(mb), get_cols(mb))){
    for(int i=0;i<(mb->rows)*(mb->cols);i++){ // como mi matriz esta alamcenada de manera que es un arreglo unidimencional, multiplico para obtener el largo exacto 
     	if (mb->data[i] - ma->data[i] != V_DELTA_PRECS){ // le coloque el distinto pero no me anda
        	return 0; //false 
        }
    }
    return 1; // true
...
bool check_cant_row(const matrix_t *ma, int row){

  return (ma && ma->rows == row);
}

bool check_cant_col(const matrix_t *ma, int col){

  return (ma && ma->cols == col);
}

bool check_dimetions(const matrix_t *ma, int row, int col){
    
    return ((check_cant_row(ma,row)) && (check_cant_col(ma, col)));
}